...
tf.app.flags.DEFINE_string("tfrecords_dir", "./tfrecords02/captcha.tfrecords", "验证码tfrecords文件")
tf.app.flags.DEFINE_string("captcha_dir", "G:\传智播客———十一之前的学习\就业班8.数据分析\data\Genpics\\", "验证码图片路径")
...
def get_captcha_label():
    ...
    :return: label
    """
    file_queue = tf.train.string_input_producer(["G:\传智播客———十一之前的学习\就业班8.数据分析\data\GenPics\lables.csv"], shuffle=False)
...
if __name__ == "__main__":
    ...
    # 获取验证码文件当中的图片
    image_batch, key, value = get_captcha_image()
    ...
    with tf.Session() as sess:
        ...
        # key = sess.run(key).eval()
        # print(key)
        #
        # value = sess.run(value).eval()
        # print(value)
